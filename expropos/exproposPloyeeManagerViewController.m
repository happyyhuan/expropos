...
//    }
    ...
    if(self.ployeeItems.count !=0)
    {
        //---初始化时候显示默认第一条数据的信息
        ExproMember *member = [self.ployeeItems objectAtIndex:0];
...
-(void)deleteSucceed:(id)sender
{
    ...
    [member deleteEntity];
    ExproMember *oldmember = [self.ployeeItems objectAtIndex:_currentPloyeeId]; 
...
- (IBAction)modifyPloyee:(id)sender {
    ...
    pay.viewController = self;
    ExproMember *member = [self.ployeeItems objectAtIndex:_currentPloyeeId];  
